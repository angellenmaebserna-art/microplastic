...
csv_files = [
    "all_dataset_csv_file/Philippine rivers and lakes.csv",
    "all_dataset_csv_file/Sitio Pulo Navotas.csv",
    "all_dataset_csv_file/Northern Laguna Lake.csv",
    "all_dataset_csv_file/Laguna de Bay.csv",
    "all_dataset_csv_file/Butuan bay.csv"
]
...
# -------------------- PREDICTIONS --------------------
    ...
    if model_choice == "Random Forest":
        ...
        try:
            ...
            X_train, X_test, y_train, y_test = train_test_split(features, df_model[target_col], test_size=test_size, random_state=42)
            ...
            col1, col2, col3 = st.columns(3)
            col1.metric("R²", f"{r2:.3f}", interpret_r2(r2))
            col2.metric("RMSE", f"{rmse:.3f}", interpret_err(rmse, y_test))
            col3.metric("MAE", f"{mae:.3f}", interpret_err(mae, y_test))

            # Feature importance
            st.subheader("🌿 Feature Importance")
            importances = pd.DataFrame({"Feature": features.columns, "Importance": rf.feature_importances_}).sort_values("Importance", ascending=False)
    ...
    # -------------------- PROPHET --------------------
        ...
        try:
            ...
            if len(prophet_df) < 10:
                ...
                col1, col2, col3 = st.columns(3)
                col1.metric("R²", f"{r2:.3f}", interpret_r2(r2))
                col2.metric("RMSE", f"{rmse:.3f}", interpret_err(rmse, y_true))
                col3.metric("MAE", f"{mae:.3f}", interpret_err(mae, y_true))
    ...
    # -------------------- SARIMA --------------------
        ...
        try:
            ...
            if best_res is not None:
                ...
                col1, col2, col3 = st.columns(3)
                col1.metric("R²", f"{r2:.3f}", interpret_r2(r2))
                col2.metric("RMSE", f"{rmse:.3f}", interpret_err(rmse, ts))
                col3.metric("MAE", f"{mae:.3f}", interpret_err(mae, ts))
